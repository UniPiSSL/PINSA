...
class CheckViolationWorkload extends WorkloadModuleBase {
    constructor() {
        super();
        this.txIndex = 0;
        this.policyholders = [];
    ...
    async submitTransaction() {
        if (this.shuffledPolicyholders.length === 0) {
            throw new Error('No policyholders available for violation check');
        }
        ...
        // Submit transaction to check obligations
...
function createWorkloadModule() {
    return new CheckViolationWorkload();
}
