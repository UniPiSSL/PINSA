...
#        log_status("(agent_container.py) - handle_issue_credential()")
...
#        log_status("(agent_container.py) - handle_issue_credential_v2_0")
        state = message.get("state")
    ...
    async def handle_present_proof_v2_0(self, message):
        ...
        if state == "request-received":
            ...
            log_status("MESSAGE=" + str(message))
            log_status("pres_request_indy=" + str(pres_request_indy))
            log_status("pres_request_dif=" + str(pres_request_dif))
    ...
    async def generate_invitation(
        ...
        with log_timer("Generate invitation duration:"):
            ...
            log_status(
                "#7 Create a connection to policyholder and print out the invite details"
            )
...
def arg_parser(ident: str = None, port: int = 8020):
    ...
    "ident", if specified, refers to one of the standard demo personas - policyholder, ncsa, insurancecompany or performance.
    """
    ...
    if (not ident) or (ident != "policyholder"):
        parser.add_argument(
            "--did-exchange",
            action="store_true",
    ...
    if (not ident) or (ident != "alice"):
        parser.add_argument(
            ...
            action="store_true",
            help=(
                "Reuse connections by using Ncsa public key in the invite. "
